...
def main():
    ...
    while run:
        lives_left = main_font.render(f"{lives}", 1, (255, 255, 255))
        clock.tick(FPS)
        redraw_window()
        ...
        if len(enemies) != wave_length and len(enemies) > 0:
        ...
        if len(enemies) == 0:
            ...
            if level > 1:
                ...
                WIN.blit(lives_left, (WIDTH // 2 - lives_left.get_width() / 2 , HEIGHT // 2 - 100))
                bonus_score = main_font.render(f"Bonus Score :{lives * 10}", 1, (255, 255, 255))
                WIN.blit(bonus_score, (WIDTH // 2 - lives_left.get_width() / 2, HEIGHT // 2 + 100))
                pygame.display.update()
                score += lives * 10
